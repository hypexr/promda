...
test('reduceGroupAsync transduce with empty data', async () => {
  ...
  const testTransducer = P.transforms.reduceGroupAsync (testValueFn, () => ([]), R.identity, [], testCompFn);
...
test('reduceGroupAsync transduce with small data', async () => {
  ...
  const testTransducer = P.transforms.reduceGroupAsync (testValueFn, () => ({ [key]: [] }), R.identity, { [key]: [] }, testCompFn);
  const result = await P.transduce (
    testTransducer,
    R.flip (R.append),
    [],
...
test('reduceGroupAsync transduce removes result reduced', async () => {
  ...
  const testTransducer = P.composeTransducer (
    ...
    P.transforms.reduceGroupAsync (testValueFn, () => ({ [key]: [] }), R.identity, { [key]: [] }, testCompFn),
  );
  const result = await P.transduce (
    testTransducer,
    R.flip (R.append),
...
test('reduceGroupAsync transduce observes reduced', async () => {
  ...
  const testTransducer = P.pipeTransducer (
    ...
    P.transforms.reduceGroupAsync (testValueFn, () => ([]), R.identity, [], testCompFn),
  );
  const result = await P.transduce (
    testTransducer,
    R.flip (R.append),
